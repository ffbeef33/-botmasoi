...
async def send_game_summary(interaction, game_state, guild_id):
    ...
    try:
        ...
        if game_logs:
            log_text = "\n".join(game_logs[-10:])  # Chỉ hiển thị 10 dòng log gần nhất
            embed.add_field(name="Diễn biến", value=log_text or "Không có dữ liệu", inline=False)
        ...
        for user_id, data in game_state["players"].items():
            ...
            if member:
                status = "Sống" if data["status"] == "alive" else "Bị thương" if data["status"] == "wounded" else "Chết"
        ...
        stats = [
            ...
            f"Số người chơi: {len(game_state['players'])}"
        ]
        embed.add_field(name="Thống kê", value="\n".join(stats), inline=False)
        
        embed.set_footer(text=f"Log được gửi bởi {interaction.user.name} | {BOT_VERSION}")
