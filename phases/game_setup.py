...
async def start_game_logic(interaction: discord.Interaction, game_state):
    ...
    if not guild:
        logger.error(f"Guild not found in interaction, ID={interaction.guild_id}")
        await text_channel.send("Lỗi: Không tìm thấy guild.")
        return
    
    if not text_channel:
        logger.error("Text channel is None, cannot proceed with game setup")
        return
    ...
    try:
        ...
        for user_id in game_state["temp_players"]:
            ...
            player_channel_tasks.append(create_player_channel(guild, channel_name, overwrites, member.id, game_state))
        ...
        start_embed.set_image(url="https://cdn.discordapp.com/attachments/1365707789321633813/1377179925517176893/Banner_early_acccess_Recovered_2160x1280_copy.png?ex=68380641&is=6836b4c1&hm=46921294b1294e4cd5fb47dd2eef4d72843effe5b1069b92bf53f68c7339f827&")  # Thêm URL của ảnh banner
...
async def start_new_game_with_same_setup(interaction: discord.Interaction, game_state):
    ...
    if not game_state["temp_players"] or not game_state["temp_roles"]:
        await interaction.channel.send("Lỗi: Không có thông tin người chơi hoặc vai trò để khởi động lại game!")
        return
    ...
    if not voice_channel:
        await interaction.channel.send(f"Lỗi: Không tìm thấy kênh voice với ID {voice_channel_id}!")
        return
    
    # Làm mới member_cache
    try:
        game_state["member_cache"] = {m.id: m for m in await retry_api_call(lambda: guild.fetch_members(limit=None).flatten())}
        ...
        await interaction.channel.send("Lỗi: Không thể làm mới danh sách thành viên server.")
        return
    ...
    if missing_players:
        missing_names = [game_state["member_cache"][uid].display_name if uid in game_state["member_cache"] else f"ID:{uid}" for uid in missing_players]
        await interaction.channel.send(f"Các người chơi sau không còn trong kênh voice: {', '.join(missing_names)}")
        
        # Tùy chọn tiếp tục hoặc hủy
        view = ContinueWithMissingPlayersView(interaction, game_state, missing_players)
        await interaction.channel.send("Bạn muốn tiếp tục game mà không có những người chơi này?", view=view)
        return
    ...
    try:
        ...
        await interaction.channel.send(f"Lỗi: Không thể tham gia kênh voice {voice_channel.name}.")
        return
    
    # Reset các giá trị cần thiết và khởi động lại game
...
class ContinueWithMissingPlayersView(discord.ui.View):
    ...
    async def continue_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if total_roles > roles_needed:
            ...
            await start_game_logic(self.interaction, self.game_state)
